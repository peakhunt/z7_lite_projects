	...
	int Direction)
{
	 ...
	 */
	if (XAxiDma_HasSg(InstancePtr)) {
		xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: Simple DMA mode is not supported\n");
	...
	if(Direction == XAXIDMA_DMA_TO_DEVICE){
		if ((Length < 1) ||
		  ...
		  xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: tx max transfer len error %d, %d\n", Length, InstancePtr->TxBdRing.MaxTransferLen);
			return XST_INVALID_PARAM;
		}

		if (!InstancePtr->HasMm2S) {
			xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: MM2S channel is not supported\n");
        ...
        xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: Engine is busy\n");
				return XST_FAILURE;
			}
		}
        ...
        xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma:Unaligned transfer without DRE %x\n",(unsigned int)BuffAddr);
		...
		if ((Length < 1) ||
		  ...
		  xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: rx max transfer len error %d, %d\n", Length, InstancePtr->RxBdRing[RingIndex].MaxTransferLen);
			return XST_INVALID_PARAM;
		}


		if (!InstancePtr->HasS2Mm) {
			xdbg_printf(XDBG_DEBUG_ERROR, "xaxidma: S2MM channel is not supported\n");
