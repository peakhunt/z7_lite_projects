...
static int
digilent_encoder_encoder_init(struct i2c_client *client,
    struct drm_device *dev,
    struct drm_encoder_slave *encoder)
{
	struct digilent_encoder *digilent = i2c_get_clientdata(client);
   ...
   digilent->i2c_present = false;
	...
	sub_node = of_parse_phandle(client->dev.of_node, "digilent,edid-i2c", 0);
	if (sub_node) 
   {
	...
	ret = of_property_read_u32(client->dev.of_node, "digilent,fmax", &digilent->fmax);
	if (ret < 0) {
      digilent->fmax = DIGILENT_ENC_MAX_FREQ;
		DRM_INFO("No max frequency in DT, using default %dKHz\n", DIGILENT_ENC_MAX_FREQ);
	}

	ret = of_property_read_u32(client->dev.of_node, "digilent,hmax", &digilent->hmax);
	if (ret < 0) {
      digilent->hmax = DIGILENT_ENC_MAX_H;
		DRM_INFO("No max horizontal width in DT, using default %d\n", DIGILENT_ENC_MAX_H);
	}

	ret = of_property_read_u32(client->dev.of_node, "digilent,vmax", &digilent->vmax);
	if (ret < 0) {
      digilent->vmax = DIGILENT_ENC_MAX_V;
		DRM_INFO("No max vertical height in DT, using default %d\n", DIGILENT_ENC_MAX_V);
	}

	ret = of_property_read_u32(client->dev.of_node, "digilent,hpref", &digilent->hpref);
	if (ret < 0) {
	...
	ret = of_property_read_u32(client->dev.of_node, "digilent,vpref", &digilent->vpref);
	if (ret < 0) {
...
static int
digilent_encoder_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	struct digilent_encoder *digilent;
  struct device *dev = &client->dev;

  printk(KERN_INFO "### digilent encoder probing\n");

	digilent = devm_kzalloc(dev, sizeof(*digilent), GFP_KERNEL);
	if (!digilent)
		return -ENOMEM;

  digilent->client = client;
  digilent->i2c_bus = client->adapter;

  i2c_set_clientdata(client, digilent);

	return 0;
}

static int
digilent_encoder_remove(struct i2c_client *client)
{
  struct digilent_encoder *digilent = i2c_get_clientdata(client);
  struct device* dev = &client->dev;

  devm_kfree(dev, digilent);

	return 0;
}

static const struct i2c_device_id digilent_encoder_of_match[] = {
	{ "digilent,drm-encoder", 0 },
	{ /* end of table */ },
};
MODULE_DEVICE_TABLE(i2c, digilent_encoder_of_match);
...
static struct drm_i2c_encoder_driver digilent_encoder_driver = {
    ...
    .id_table = digilent_encoder_of_match,
	},
